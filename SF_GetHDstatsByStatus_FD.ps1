...
$statuts = @("Tous","Ouvert","Arrives a echeance","Echeance aujourd'hui","En attente","Nouveau")

##################################################
$output+="<?xml version=`"1.0`" encoding=`"UTF-8`" ?>"
$output+="<prtg>"

# Finding the position of the parameter in the list
$Count = 0
foreach ($status in $statuses)
{
    if ($status.ToString() -eq $args[0].ToString()) {break}
    
    $Count++

}

Write-Host $statuses[$Count]
# The Doing part
$FDBaseEndpointSummary =  "https://support.sensorfactory.eu/helpdesk/tickets/summary.xml?view_name=" + $statuses[$Count].ToString()
